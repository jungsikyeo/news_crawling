...
- **스케줄 수집** — 5분~24시간 간격으로 자동 반복 수집
- **즉시 수집** — 시작일 지정 가능한 일회성 수집 (스케줄과 독립 실행)
- **OS 알림** — 수집 완료 시 Windows 네이티브 알림
- **시스템 트레이** — 트레이 아이콘 상주, 우클릭 메뉴 (열기/종료)
...
│           └── useApi.ts     # API 클라이언트
├── electron/
│   ├── main.js              # Electron 메인 프로세스 (트레이, 알림, IPC)
