...
import { fetchHistory, deleteHistory } from "@/hooks/useApi"
      ...
      <div className="flex flex-col gap-3">
        {items.map((item, index) => {
          ...
          return (
            <div
              key={item.id}
              className="card-hover rounded-xl border border-border/50 bg-card px-4 py-3 flex items-center animate-fade-up"
              style={{ "--delay": `${index * 50}ms` } as React.CSSProperties}
            >
              {/* Left: keyword, portals */}
                  ...
                  {portalList.map((p) =>
                    PORTAL_IMGS[p] ? (
                      <img
                        key={p}
                        src={PORTAL_IMGS[p]}
                        alt={PORTAL_LABELS[p] ?? p}
                        title={PORTAL_LABELS[p] ?? p}
